...
def save_csv(path_XYs, csv_path):
    ...
    for path in path_XYs:
        for line_id, line in enumerate(path):
            for point in line:
                data.append([path_id, line_id, point[0], point[1]])
        path_id += 1
    np.savetxt(csv_path, data, delimiter=",", fmt="%d,%d,%f,%f")
...
for contour in contours:
    ...
    output_polylines.append(approx.reshape(-1, 2))

save_csv([output_polylines], 'solutions/isolated.csv')
cv2.imwrite('solutions/isolated.png', image)
